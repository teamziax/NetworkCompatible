...
/** WS2 adapter: background key/profile lifecycle only; no per-join metadata input is used. */
public final class NativeProviderTransport implements ProviderTransport {
    public static final String CAPABILITY = "dev.warden.stateless-admission.v2";
    ...
    private NativeProviderTransport(NativeAdmissionServerChannel channel, StatelessAdmissionValidator validator, String incarnation) {
    ...
    public static CompletionStage<NativeProviderTransport> open(ServerBootstrap bootstrap, InetSocketAddress bind, Path certificate, Path privateKey, AdmissionGate.Limits limits) {
        ...
        try {
            ...
            var endpoint = new NativeAdmissionServerChannel(identity, validator, limits);
            bootstrap.clone().channelFactory(() -> endpoint).bind(bind).addListener(future -> {
                if (future.isSuccess()) result.complete(new NativeProviderTransport(endpoint, validator, incarnation));
    ...
    public static String audience(String incarnation) {
        ...
        return "warden-stateless-host-v2/" + incarnation;
    }
    ...
    @Override public synchronized CompletionStage<JsonObject> hostProfile() {
        ...
        // WS2 supplies keys oldest-to-newest and acknowledges its last epoch before publication.
        ...
        InetSocketAddress bind = (InetSocketAddress)channel.localAddress();
