...
/** WDA2 validation using only a background key snapshot and raw client STUN. */
public final class StatelessAdmissionValidator implements AdmissionValidator {
    ...
    public synchronized void installKeys(List<TicketKey> snapshot) {
        ...
        for (TicketKey key : snapshot) {
            ...
            next.put(key.keyId(), new Material(hmac("HmacSHA256", secret, utf8("warden-stateless-aead-v2\0" + audience)), secret, key.notBefore(), key.retireAfter()));
        }
    ...
    @Override public synchronized VerifiedAdmission validate(byte[] packet, StunBinding binding, long nowMillis) {
        ...
        try {
            ...
            if (token.length() < 8 || !token.startsWith("WDA2")) return null;
            ...
            cipher.updateAAD(utf8("warden-stateless-admission-v2\0" + token.substring(0, 8) + "\0" + audience + "\0" + binding.remoteUfrag()));
            ...
            String localPassword = BASE64.encodeToString(Arrays.copyOf(hmac("HmacSHA256", key.secret(), utf8("warden-stateless-ice-v2\0" + audience + "\0" + token)), 24));
